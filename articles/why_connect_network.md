...
## 5.5 コンテンツ配信サービス
**キャッシュ・サーバーをインターネット上に設置してWebサービス事業者に貸し出す**というサービスが、コンテンツ配信サービス。
クライアント側のLAN内でも、サーバー側のLAN内でもなく、インターネット上にあることで、インターネットのトラフィックを抑える効果がある。（詳しくは本書P370）
...
### <最寄りのキャッシュ・サーバーの見つけ方>
2通りある。
**TODO: 詳細まで理解できなかったため、後で調べながら理解すること。**
1. 全キャッシュ・サーバーの設置場所にあるルーターから入手した経路表を、DNSサーバーに登録する方法。
2. リダイレクト用サーバーでアクセス先を振り分ける。
