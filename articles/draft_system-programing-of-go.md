...
OS は、システムコール（例えば1章の`syscall.Write()`）を呼び出すとき、ファイルディスクリプタ（`1`, `2`などの識別子）を指定する。
...
OSのレイヤー（Goより下）では、ファイルディスクリプタを使って、色々なモノをファイルとして抽象化しているが、
Goのレイヤーでは、io.Writerインターフェースが抽象化して表現している。
