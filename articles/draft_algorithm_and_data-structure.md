...
:::message
配列（dpと書くことが多い）に、その部分問題の最適解をメモして、順に決定していく。
:::
...
```
実装したことで、挿入ソートの下記の特徴がより体感的に理解できる。
- 計算量は`O(N^2)`
- in placeなソート
- 安定ソート

#### マージソート
配列を左右に2分割する ということを、それ以上分割できなくなるまで（= 分割すると空になるまで）繰り返す。
それに対してソートしてそれらをマージする ということを、元の1つの配列に戻るまで繰り返す。
（分割統治法）

- 計算量は`O(NlogN)`
- 外部のメモリ空間が必要なのでin placeでない。
- 安定ソート
