...
（感想：これは自分には無かった観点だった。変更や拡張はこれからのエンジニア人生で多くの割合を占めるはずなので、これからこの観点を意識する。）
:::
...
メソッドに対しても、**その役割を1文で説明**できるようにすること。
...
## 送信メッセージをテストする
### 送信クエリメッセージ
副作用がないため、テストするべきでない。(selfに送られたメッセージも)
### コマンドメッセージ
アプリケーションの一部が、その結果として生じる何かに依存する場合は、テストする必要がある。