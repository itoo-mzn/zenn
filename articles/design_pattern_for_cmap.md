...
1. このサイトをinputに、全部をざっくり理解する。

https://xtech.nikkei.com/it/article/COLUMN/20051123/225074/

2. 3つに上げる候補を列挙。 →14個候補になった。

3. このサイトをinputに、14→6個ぐらいに絞る。
  自分が興味があるorどこかで使いそうだと思えるもの。

https://refactoring.guru/ja/design-patterns

4. その6個を、そのサイトに書いているように、typescriptで書いてみて、理解する。

5. 3つのcmapへの応用パターンを考えて、typescriptで書く。
  （ここでやることは、別記事にする。この記事の立ち位置は「デザインパターン全体を軽く理解+興味があるものをピックアップしてコードを書いてみた」というものにする。）

6. 残りのパターンについて、自分の言葉で概要をまとめる。
...
### Composite（合成物）パターン ☆5
  `個々のオブジェクト`と`合成したオブジェクト`を同一視することにより、再帰的な構造を表現します。

  インターフェースを作り、それを`オブジェクト単体`と`それら複数から成る階層構造を持ったオブジェクト`に適応し、実装を強制させる。
  オブジェクト単体では、単に自己完結型で処理を行うよう実装。
  合成オブジェクトでは、単体オブジェクトにメソッドを実行させて、その後、次に処理すべきオブジェクトがあれば再帰的にメソッドを呼び出す。
  それらのメソッドが、同じインターフェースで実装されているので呼び出し方が同じなのが良い点。
...
### Interpreter（通訳）パターン ☆3
  文法規則をクラスで表現したものを用い構文解析し、その結果に基づき処理を実行していく機能を提供します。

  滅多に必要とされないパターン。

### Template Method（ひな型メソッド）パターン ☆5
...
### Command（命令）パターン ☆3
  ...
  注文メソッドの引数に、具体的な具材・調理方法を渡すのでなく、作ってほしい料理名を渡す。
...
### Memento（形見）パターン ☆4
  ...
  （railsでいうと、change_カラム名みたいなメソッドがしていることが似ている。）
