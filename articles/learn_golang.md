...
## 課題（メソッドとインターフェイス）
```go:main.go
// アカウント所有者の姓と名を含む、Account という名前のカスタム型を作成します。
// 型には、ChangeName の機能も含まれている必要があります。
type Account struct {
	firstName string
	lastName  string
}
...
func (a *Account) ChangeName(name string) {
	a.firstName = name // lastNameどうする？
}

// クレジット数を float64 型として格納するための変数を含み Account オブジェクトを埋め込む、Employee という名前の別のカスタム型を作成します。
// 型には AddCredits、RemoveCredits、CheckCredits の機能も含まれている必要があります。
// Employee オブジェクトを使用してアカウント名を変更できることを示す必要があります。
type Employee struct {
	Account
	credit float64
}

func (e *Employee) AddCredits() int {
	e.credit++
	return int(e.credit)
}

// RemoveCredits、CheckCredits とは？
...
func main() {
	account := Account{firstName: "田中", lastName: "一郎"}
	fmt.Println(account) // 田中 一郎

	account.ChangeName("山本")
	fmt.Println(account) // 山本 一郎

	employee := Employee{Account{firstName: "佐藤", lastName: "二郎"}, 123}
	fmt.Println(employee) // 佐藤 二郎 (Stringerにより出力が{{佐藤 二郎} 123}から変化)
	fmt.Println(employee.credit) // 123
}
